...
const PromotionalVideosPage: React.FC = () => {
  ...
  const [pageSize, setPageSize] = useState<number>(9); // 9 videos per page (3x3 grid)
  ...
  const fetchVideos = useCallback(async () => {
    try {
      ...
      const { data } = await axiosInstance.get("/api/admin/promotional-videos", {
        params: { skip, limit: pageSize }, // Pass skip and limit as query params
      });
      setVideos(data.videos);
      setTotalVideos(data.total || 0); // Assuming API returns total count
    } catch (error: any) {
      message.error(error.response?.data?.error || "Failed to fetch promotional videos");
    } finally {
      setFetching(false);
    }
  ...
  const handleAddOrUpdateVideo = async (values: any) => {
    try {
      ...
      if (editingVideo) {
        ...
        setTotalVideos((prev) => prev + 1); // Update total count
      ...
      message.error(
        error.response?.data?.error || error.message || "Failed to save promotional video"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleHidden = async (id: string, hidden: boolean) => {
    try {
      ...
      message.error(error.response?.data?.error || "Failed to toggle visibility");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteVideo = async (id: string) => {
    try {
      ...
      message.error(error.response?.data?.error || "Failed to delete promotional video");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
          ...
          >
            ...
            >
              <Avatar className="object-cover w-full h-full rounded-none">
                <AvatarImage src={`/gcs/${video.videoId}/thumbnails/thumb00001.jpg`} />
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="flex flex-col w-full">
                  <div className="flex justify-between text-xs text-gray-500">
                    <span className="font-medium text-neutral-800">{video.createdBy}</span>
      ...
      <div className="mt-8 flex justify-center">
        <Pagination
          ...
          showSizeChanger
          pageSizeOptions={["1", "9", "18", "27"]} // Options for videos per page
      ...
      {/* Create/Edit Modal */}
      <Modal
        title={editingVideo ? "Edit Promotional Video" : "Create Promotional Video"}
        open={isModalVisible}
        onCancel={() => {
          setIsModalVisible(false);
          setEditingVideo(null);
      ...
      >
        {editingVideo && editingVideo.hslUrl && (
          <div className="mb-6">
            <h3 className="text-lg font-semibold">{editingVideo.title || "Untitled"}</h3>
            <Player
        ...
        >
          ...
          <Row gutter={16}>
            <Col span={12}>
              ...
              >
                <InputNumber min={0} style={{ width: "100%" }} className="rounded-full" />
              </Form.Item>
            </Col>
            <Col span={12}>
              {!editingVideo && (
